...
class SpaceNE(object):
    ...
    def train(self):
        ...
        # optimization setup
        # train_step = tf.optimizers.Adam(0.01).minimize(objective_loss,var_list = [])
        train_step = tf.keras.optimizers.Adam(0.01)
    ...
    def within_class_dis(self):
        ...
        for i in range(self.num_community):
            part1 = tf.matmul(tf.transpose(a=self.U[i]), tf.transpose(a=self.X[i]))
            part2 = tf.matmul(self.X[i], self.U[i])
            sim_inner -= tf.linalg.trace(tf.matmul(part1, part2))
        return sim_inner
    ...
    def between_class_dis(self):
        ...
        for i in range(self.num_community):
            for j in range(i + 1, self.num_community):
                ...
                part1_2 = tf.matmul(tf.transpose(a=self.U[j]), tf.transpose(a=self.X[j]))
                part1 = tf.matmul(part1_1, part1_2)
                part2 = tf.matmul(self.X[i], tf.transpose(a=self.X[j]))
                sim_external += tf.norm(tensor=tf.subtract(part1, part2), ord='euclidean')  # F
        return sim_external
    ...
    def rank_approximation(self, X):
        ...
        res = tf.reduce_sum(input_tensor=tf.compat.v1.where(s > divide_point, cal2(s, divide_point), cal1(s, divide_point)))
    ...
    def load_W(self):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            W = sess.run(tf.compat.v1.get_default_graph().get_tensor_by_name('W:0'))  # the learnt projection matrix
        return W
