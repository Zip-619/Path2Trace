...
// degrees[i], &weight_js[offsets[i]], &weights[offsets[i]]
// weight_js 是原始的weight
// weights 是归一化之后的weight

void init_walker(int n, int *j, float *probs) { // 采用的是alias sampling。j中存的是表中第i列不是事件i的另一个事件标号，
//probs存储的是i列中i事件对应的概率，采样过程使用首先采样某一列，再采样某一个特定的元素
  ...
  while (smaller.size() != 0 && larger.size() != 0) {
    ...
    larger.pop_back();
    j[small] = large;
    probs[large] += probs[small] - 1; // prob  large-1+small
    if (probs[large] < 1)
...
#pragma omp atomic
      ...
      update(&w0[n1 * n_hidden], &w1[n2 * n_hidden], 1, nce_bias);
      for (int i = 0; i < n_samples; i++) {
        size_t neg = irand(nv);
        update(&w0[n1 * n_hidden], &w1[neg * n_hidden], 0, nce_bias_neg);
      }
      ncount++;
    }
  }
...
int main(int argc, char **argv) {
  int a = 0;
  string network_file, embedding_file, pre_emb_file;
  bool init = true;
  ...
  if ((a = ArgPos(const_cast<char *>("-init"), argc, argv)) > 0)
  {
    cout<<"init 赋值";
    // init = bool(argv[a+1]);
    string temp = argv[a+1];
    string f = "false";
    string t = "true";
    if (temp.compare(f)==0){
      init = false;
  ...
  // if ((a = ArgPos(const_cast<char *>("-pre_emb_file"), argc, argv)) > 0)
  //   pre_emb_file = argv[a+1];
  cout<<init<<endl;
  ...
  if (embFile.is_open()) {
    ...
    offsets = static_cast<int *>(aligned_malloc((nv + 1) * sizeof(int32_t), DEFAULT_ALIGN)); //存储当前节点之前所有边数之和，其中最后一个元素是offsets[-1] = ne
    edges = static_cast<int *>(aligned_malloc(ne * sizeof(int32_t), DEFAULT_ALIGN)); // 存储每个to_node,这个和offset配合使用
    weights = static_cast<float *>(aligned_malloc(ne * sizeof(float), DEFAULT_ALIGN)); // 存储跟edge对应的每条边的权重
  ...
  weight_js = static_cast<int *>(aligned_malloc(ne * sizeof(int32_t), DEFAULT_ALIGN));//static_cast是C++中标准的强制转换
  ...
  if (init==true){
    ...
    cout<<"在已经获得embedding上优化"<<endl;
    ifstream w0_emb_file(embedding_file+"_s", std::ios::in | std::ios::binary);
    ifstream w1_emb_file(embedding_file+"_t", std::ios::in | std::ios::binary);
    w0_emb_file.read(reinterpret_cast<char *>(w0), sizeof(float) * n_hidden * nv);  
    w1_emb_file.read(reinterpret_cast<char *>(w1), sizeof(float) * n_hidden * nv);
  }
  // 初始化的全部为内存中的值,可能全为0.所以通过下边来进行随机的初始化
  
  degrees = (int *)malloc(nv * sizeof(int)); //存储每个节点所有的出度
  for (int i = 0; i < nv; i++)
  ...
  for(int i=0;i<nv;i++)
  {
    ...
    for(int j=offsets[i];j<offsets[i+1];j++)
      sum += weights[j];
    sum /= degrees[i];
    for(int j=offsets[i];j<offsets[i+1];j++) //将weight做归一化处理
      weights[j] /= sum;
